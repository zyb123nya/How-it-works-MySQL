...
void auto_complete(InputBuffer *input_buffer) {
    // 定义支持的命令列表
    ...
    for (int i = 0; i < sizeof(commands) / sizeof(commands[0]); i++) {
        ...
        if (strncmp(input_buffer->buffer, cmd, strlen(input_buffer->buffer)) == 0) {
            // 找到匹配的命令，将其补全到输入缓冲区
            strncpy(input_buffer->buffer, cmd, input_buffer->buffer_length);
            input_buffer->input_length = strlen(input_buffer->buffer);
            printf("%s", cmd); // 打印补全的命令
            break;
        }
    }
}
